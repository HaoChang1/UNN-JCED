...
from MatlabFuncHelper import *
from OTFS_ResGrid import OTFSResGrid
...
class OTFS(MatlabFuncHelper):
    ...
    '''
    def setChannel(self, in1, in2, in3, *, force_frac=False, isNorm=False,isAWGN=False, isRician=False, isUniform=False,isTDL_A=False,isTDL_B=False,isTDL_C=False,isTDL_D=False,isTDL_E=False,isEVA=False,isETU=False,isexp=False):
        in1 = np.asarray(in1);
        in2 = np.asarray(in2);
        in3 = np.asarray(in3);
        # set the channel
        if in1.ndim > 0 and in2.ndim > 0 and in3.ndim > 0:
            ...
            l_combs = kron(np.arange(0, lmax + 1), np.ones((1, kmax - kmin + 1))).astype(int);
            ...
            if isAWGN:
                ...
                tdl_power = np.array([0, -4.8,-7.6,-9.8])  
                tdl_taps = tdl_power[0:np.max(self.delay_taps)+1] 
                path_power = np.power(10,tdl_taps/10)
                channel_power = np.choose(self.delay_taps, path_power) 
                if isNorm == True:
                    path_power_sum = np.sum(channel_power,axis=-1)
                    channel_power =  channel_power/np.repeat(np.expand_dims(path_power_sum, axis=-1),p,axis=-1) 
                self.chan_coef = (np.random.randn(self.batch_size,int(p))+1j*np.random.randn(self.batch_size,int(p)))*np.sqrt(channel_power/2)    
              
            elif isTDL_E: 
                ####LOS path -0.03dB
                tdl_power = np.array([-22.03,-22.4,-22.6,-22.3])
                tdl_taps = tdl_power[0:np.max(self.delay_taps)+1] 
                path_power = np.power(10,tdl_taps/10)
                LoS_power = np.power(10,-0.03/10)
                zero_rows,zero_cols = np.where(self.delay_taps == 0) 
                unique_rows,first_indices = np.unique(zero_rows, return_index=True)
                unique_cols = zero_cols[first_indices] 
                channel_power  = np.choose(self.delay_taps, path_power)   
                self.chan_coef = (np.random.randn(self.batch_size,int(p))+1j*np.random.randn(self.batch_size,int(p)))*np.sqrt(channel_power/2)
                Los_chan_coef = (1+1j)*np.sqrt(LoS_power/2) 
                self.chan_coef[unique_rows,unique_cols] = Los_chan_coef  
                  
            else:
                self.chan_coef = sqrt(1/2/p)*(self.randn(p)+1j*self.randn(p)); # use Rayleigh fading by default
                # self.chan_coef = sqrt(1/2)*(self.randn(p)+1j*self.randn(p));
            self.cp_len = np.max(self.delay_taps).astype(int);
        else:
    ...
    def passChannel(self, No):
        ...
        elif No.shape[0] == self.batch_size:
            No = No
        else:
        ...
        if No.ndim == 0:
            ...
            No_re = np.repeat(No.reshape(-1,1),self.sig_len,axis=-1)
            noise = sqrt(No_re/2)*(self.randn(self.sig_len) + 1j*self.randn(self.sig_len));
            s_chan = s_chan + noise;
        elif self.isvector(No):
            s_chan = s_chan + No;
    ...
    def getChannel(self, *args, data_only=True):
        ...
        if data_only and self.rg is not None:
            H_DD,col_ids,row_ids = self.removeNoDAChannel(H_DD);
 
        return H_DD,col_ids,row_ids
    ...
    def buildPhi_h(self,lmax, kmax,hi,li,ki,X_p):
        h = np.zeros((self.batch_size,2*kmax+1,lmax+1), dtype=complex)
        for bb in range(self.batch_size):
            for indx in range(hi.shape[1]):
                k_ind = ki+kmax
                h[bb,k_ind[bb,indx],li[bb,indx]] = hi[bb,indx]
        ...
        Phi = np.zeros((self.batch_size,self.sig_len, pmax), dtype=complex)  # the return matrix
        for aa in range(self.batch_size):
            for yk in range(1, self.nTimeslotNum + 1):
                for yl in range(1, self.nSubcarNum + 1):
                    ...
                    for p_id in range(pmax):
                        ...
                        if yl - 1 < li:
                            xl += self.nSubcarNum
                        xk = (yk - 1 - ki) % self.nTimeslotNum + 1
                        if self.pulse_type == self.PULSE_IDEAL:
                            pss_beta = np.exp(-2j * np.pi * li * ki / self.nSubcarNum / self.nTimeslotNum)
                        elif self.pulse_type == self.PULSE_RECTA:
                            pss_beta = np.exp(2j * np.pi * (yl - li - 1) * ki / self.nSubcarNum / self.nTimeslotNum)
                            if yl - 1 < li:
                                pss_beta *= np.exp(-2j * np.pi * (xk - 1) / self.nTimeslotNum)
                        Phi[aa,Phi_ri - 1, p_id] = X[aa,xk - 1, xl - 1] * pss_beta
    ...
    #     pmax = (lmax + 1) * (2 * kmax + 1)  # the number of all possible paths
    #     # lis = np.kron(np.arange(lmax + 1), np.ones(2 * kmax + 1, dtype=int))  # the delays on all possible paths
    #     kis = np.kron(np.arange(-kmax, kmax + 1), np.ones(lmax + 1, dtype=int))  # the delays on all possible paths
    #     lis = np.tile(np.arange(lmax + 1), 2*kmax + 1)  # the dopplers on all possible paths
    #     Phi = np.zeros((self.batch_size,self.sig_len, pmax), dtype=complex)  # the return matrix
    #     for aa in range(self.batch_size):
    #         for yk in range(1, self.nSubcarNum + 1):
    #             for yl in range(1, self.nTimeslotNum + 1):
    #                 Phi_ri = (yk - 1) * self.nSubcarNum + yl  # row id in Phi
    #                 for p_id in range(pmax):
    #                     li = lis[p_id]
    #                     ki = kis[p_id]
