...
class OTFSResGrid(MatlabFuncHelper):
        ...
        if in1.ndim == 0:
            if len(args) < 1:
                ...
                if in2.ndim > 0:
                    ...
                    self.content = self.zeros(self.nTimeslotNum, self.nSubcarNum).astype(np.complex128);
        else:
    ...
    def setGuard(self, *args, guard_delay_full=False, guard_doppl_full=False):
        ...
        # self.gl_len_neg = ins[4];
        # self.gl_len_pos = ins[4];
        # self.gk_len_neg = ins[5];
        # self.gk_len_pos = ins[5];
    ...
    def insertP(self, pilots_pow):
        ...
        if self.p_len == 0:
            ...
            if self.p_len > 0:
                ...
                self.pilots = sqrt(pilots_pow)*(1+1j)/np.sqrt(2)*np.ones(self.p_len);
        # allocate pilots 
        if self.p_len != 0:
            # allocate pilots
            if self.batch_size == self.BATCH_SIZE_NO:
                ...
                self.X_p = np.zeros(self.content.shape)+1j*np.zeros(self.content.shape)
                self.X_p[self.pk1:self.pk1+self.pk_len, self.pl1:self.pl1+self.pl_len] = self.content[self.pk1:self.pk1+self.pk_len, self.pl1:self.pl1+self.pl_len]
                ...
                self.X_p[..., self.pk1:self.pk1+self.pk_len, self.pl1:self.pl1+self.pl_len] =  pilots
